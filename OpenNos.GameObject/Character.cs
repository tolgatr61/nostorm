    ...
    public class Character : CharacterDTO
    {
        ...
        public void CharacterLife()
        {
            switch(Prestige)
            {
                case 1:
                    {
                        if (!Buff.ContainsKey(3000))
                        {
                            AddBuff(new Buff(3000, Level), BattleEntity);
                        }
                        if (!Buff.ContainsKey(109))
                        {
                            AddBuff(new Buff(109, Level), BattleEntity);
                        }
                        if (!Buff.ContainsKey(132))
                        {
                            AddBuff(new Buff(132, Level), BattleEntity);
                        }
                    }
                    break;
                case 2:
                    {
                        if (!Buff.ContainsKey(738))
                        {
                            AddBuff(new Buff(738, Level), BattleEntity);
                        }
                        if (!Buff.ContainsKey(3001))
                        {
                            AddBuff(new Buff(3001, Level), BattleEntity);
                        }
                        if (!Buff.ContainsKey(3000))
                        {
                            AddBuff(new Buff(3000, Level), BattleEntity);
                ...
                case 3:
                    {
                        if (!Buff.ContainsKey(685))
                        {
                            AddBuff(new Buff(685, Level), BattleEntity);
                        }
                        if (!Buff.ContainsKey(738))
                        {
                            AddBuff(new Buff(738, Level), BattleEntity);
                ...
                case 4:
                    {
                        if (!Buff.ContainsKey(18))
                        {
                            AddBuff(new Buff(18, Level), BattleEntity);
                        }
                        if (!Buff.ContainsKey(685))
                        {
                            AddBuff(new Buff(685, Level), BattleEntity);
                        ...
                        if (!Buff.ContainsKey(120))
                        {
                            AddBuff(new Buff(120, Level), BattleEntity);
                        }
                        if (!Buff.ContainsKey(132))
                        {
                            AddBuff(new Buff(132, Level), BattleEntity);
                ...
                case 5:
                    {
                        if (!Buff.ContainsKey(35))
                        {
                            AddBuff(new Buff(35, Level), BattleEntity);
                        }
                        if (!Buff.ContainsKey(728))
                        {
                            AddBuff(new Buff(728, Level), BattleEntity);
                        }
                        if (!Buff.ContainsKey(18))
                        {
                            AddBuff(new Buff(18, Level), BattleEntity);
                        ...
                        switch (specialist.Design)
                        {
                            ...
                            case 11:
                                if (!Buff.ContainsKey(410))
                                {
                                    ...
                                    //AddBuff(new Buff(5000, Level), BattleEntity, true);
                                    //int rn = ServerManager.RandomNumber();
                                    //if (rn <= 25)
                                    //{
                                    //    Console.WriteLine($"höhö 25% und buff wurde gegeben");
                                    //    AddBuff(new Buff(9415, 1), BattleEntity);
                                    //}
                                    ////if(1)
                                    ////{
                                    ////    Session.Character.FoodHp += 7777;
                                    ////    Session.Character.FoodMp += 7777;
                                    ////    Session.Character.Hp += 7777;
                                    ////}
                                }
                                break;
                            ...
                            case 15:
                                ...
                                break;
                            //case 16:
                            //    if (!Buff.ContainsKey(lol))
                            //    {
                            //        AddBuff(new Buff(lol, Level), BattleEntity, true);
                            //    }
                            //    break;
                        }
                    }
        ...
        public string GenerateIn(bool foe = false, AuthorityType receiverAuthority = AuthorityType.User, int InEffect = 0)
        {
            ...
            if (Authority > AuthorityType.User && !Undercover)
                name = $"[{Authority}]" + Name;
            if (Prestige > 0)
                name = $"[P{Prestige}]{name}";

            if (receiverAuthority >= AuthorityType.TMOD)
            {
                foe = false;
                name = $"[{Faction}][{Authority}][P{Prestige}]{ name}";
            }
        ...
        void GenerateDamageMessage(IDictionary<BattleEntity,long> dmglist, double MaxHp = 1)
        {
            var DamageList = dmglist.ToList();
           foreach(KeyValuePair<BattleEntity,long> Dmg in DamageList)
           {
                Character character = Dmg.Key.Character;
                long dmgvalue = (long)((Dmg.Value/MaxHp)*100);
                /*
                if(dmgvalue >= 8)
                {
                    Session.Character.Inventory.AddNewToInventory(4262, 1, InventoryType.Equipment);
                }
                */
                if (dmgvalue <= 3) //TODO: YUKIO
                {
                    Session.CurrentMapInstance?.Broadcast(Session, Session.Character.GenerateSay($"I did {dmgvalue} % dmg", 0), ReceiverType.All); //Sollte so funzen
                    //Session.SendPacket(Session.Character.GenerateSay("I did {dmgvalue} % dmg", 12));
                    character.Session.SendPacket(character.GenerateSay($"You didn't deal enough dmg (Minimum dmg is 3% in order to drop something) You did  {dmgvalue} % dmg", 12));
                }
                else
                {
                    Session.CurrentMapInstance?.Broadcast(Session, Session.Character.GenerateSay($"I did {dmgvalue} % dmg", 0), ReceiverType.All);
                    //Session.SendPacket(Session.Character.GenerateSay("I did {dmgvalue} % dmg", 12));
                    character.Session.SendPacket(character.GenerateSay($" You did  {dmgvalue} % dmg", 12));
                }
            }
        ...
        public void GenerateKillBonus(MapMonster monsterToAttack, BattleEntity Killer)
        {
            ...
            lock (_syncObj)
            {
                ...
                if (Session.HasCurrentMapInstance && ((MapInstance.MapInstanceType == MapInstanceType.BaseMapInstance || MapInstance.MapInstanceType == MapInstanceType.LodInstance) || MapInstance.DropAllowed))
                {
                    ...
                    if (monsterToAttack.Monster.MonsterType != MonsterType.Special)
                    {
                        ...
                        /* int dropRate = (ServerManager.Instance.Configuration.RateDrop + MapInstance.DropRate);
                         ...
                         //Für jeden Drop in Droplist die Random Geordnet ist
                               ...
                               //  double divider = !divideRate ? 1D : levelDifference >= 20 ? (levelDifference - 19) * 1.2D : levelDifference <= -20 ? (levelDifference + 19) * 1.2D : 1D;
                                 ...
                                 if (rndamount <= (double)drop.DropChance * dropRate / 1000.000)
                                 {
                                     ...
                                     if (Session.CurrentMapInstance != null)
                                     {
                                         ...
                                         lock (monsterToAttack.DamageList)
                                         {
                                             GenerateDamageMessage(monsterToAttack.DamageList,monsterToAttack.MaxHp);
                                             //min dmg
                                             if(monsterToAttack.MonsterVNum == 1058)
                                             {
                                                 int damage = Convert.ToInt32((monsterToAttack.MaxHp/100)*3);
                                                 hitters = monsterToAttack.DamageList.Where(s => s.Key?.Character != null && s.Key.Character.MapInstance == monsterToAttack.MapInstance && s.Value > damage).Select(s => s.Key.Character).ToList();
                                         ...
                                         //für jeden character mit mehr als 1 dmg bzw. mehr als 3% dmg
                                 ...
                                 {
                                     rndamount -= (double)drop.DropChance * dropRate / 1000.000;
                                 }
                             }
                         }*/
                        ...
                        List<Character> hitters = new List<Character>();
                        if (monsterToAttack.MonsterVNum == 1058 || monsterToAttack.MonsterVNum == 2553 || monsterToAttack.MonsterVNum == 2568 || monsterToAttack.MonsterVNum == 2552 || monsterToAttack.MonsterVNum == 2514 || monsterToAttack.MonsterVNum == 2504 || monsterToAttack.MonsterVNum == 2326)//2514 2504 2326
                        {
                            int damage = Convert.ToInt32((monsterToAttack.MaxHp / 100) * 3);
                            hitters = monsterToAttack.DamageList.Where(s => s.Key?.Character != null && s.Key.Character.MapInstance == monsterToAttack.MapInstance && s.Value > damage).Select(s => s.Key.Character).ToList();
                            GenerateDamageMessage(monsterToAttack.DamageList, monsterToAttack.MaxHp);
                        ...
                        foreach (Character character in hitters)
                        {
                                ...
                                if (x < 4)
                                {
                                    ...
                                    if (character.Session.CurrentMapInstance != null)
                                    {
                                        ...
                                        if (rndamount <= (double)drop.DropChance * dropRate / 1000.000 /* / divider*/)
                                        {
                                            ...
                                            character.GiftAdd(drop.ItemVNum, (byte)drop.Amount);
                                        }
                            ...
                            if (monsterToAttack.MonsterVNum == 2568)
                            {
                                int drop = ServerManager.RandomNumber(1, 2000);
                                int drop2 = ServerManager.RandomNumber(1, 100);
                                int drop3 = ServerManager.RandomNumber(1, 2000);
                                if(drop3 <= 7)
                                {
                                    character.Inventory.AddNewToInventory(1785, 1, InventoryType.Main, 0, 0, 0);
                                }
                                if (drop <= 5)
                                {
                                                                    
                                   
                                    if (character.Class == ClassType.Swordsman)
                                    {
                                        if (drop2 < 33)
                                            character.Inventory.AddNewToInventory(4981, 1, InventoryType.Equipment, 0, 0, 0);
                                        if (drop2 > 33 && drop2 < 66)
                                            character.Inventory.AddNewToInventory(4978, 1, InventoryType.Equipment, 0, 0, 0);
                                        if (drop2 > 66)
                                            character.Inventory.AddNewToInventory(4984, 1, InventoryType.Equipment, 0, 0, 0);
                                    }
                                    if (character.Class == ClassType.Archer)
                                    {
                                        if (drop2 < 33)
                                            character.Inventory.AddNewToInventory(4983, 1, InventoryType.Equipment, 0, 0, 0);
                                        if (drop2 > 33 && drop2 < 66)
                                            character.Inventory.AddNewToInventory(4980, 1, InventoryType.Equipment, 0, 0, 0);
                                        if (drop2 > 66)
                                            character.Inventory.AddNewToInventory(4986, 1, InventoryType.Equipment, 0, 0, 0);
                                    }
                                    if (character.Class == ClassType.Magician)
                                    {
                                        if (drop2 < 33)
                                            character.Inventory.AddNewToInventory(4982, 1, InventoryType.Equipment, 0, 0, 0);
                                        if (drop2 > 33 && drop2 < 66)
                                            character.Inventory.AddNewToInventory(4979, 1, InventoryType.Equipment, 0, 0, 0);
                                        if (drop2 > 66)
                                           character.Inventory.AddNewToInventory(4985, 1, InventoryType.Equipment, 0, 0, 0);
                                    }
                                }
                            }
                        }
                ...
                if (Hp > 0 && !monsterToAttack.BattleEntity.IsMateTrainer(monsterToAttack.MonsterVNum))
                {
                    ...
                    //disc
                    sbyte rndRare = (sbyte)ServerManager.RandomNumber(4, 8);
                    int rndShell = ServerManager.RandomNumber(1, 8);
                    if(monsterToAttack.MonsterVNum == 606)
                    {
                        switch(rndShell)
                        {
                            case 1:
                                Session.Character.Inventory.AddNewToInventory(571, 1, InventoryType.Equipment, rndRare, 0, 0);
                                break;
                            case 2:
                                Session.Character.Inventory.AddNewToInventory(568, 1, InventoryType.Equipment, rndRare, 0, 0);
                                break;
                            case 3:
                                Session.Character.Inventory.AddNewToInventory(574, 1, InventoryType.Equipment, rndRare, 0, 0);
                                break;
                            case 4:
                                Session.Character.Inventory.AddNewToInventory(580, 1, InventoryType.Equipment, rndRare, 0, 0);
                                break;
                            case 5:
                                Session.Character.Inventory.AddNewToInventory(583, 1, InventoryType.Equipment, rndRare, 0, 0);
                                break;
                            case 6:
                                Session.Character.Inventory.AddNewToInventory(586, 1, InventoryType.Equipment, rndRare, 0, 0);
                                break;
                            case 7:
                                Session.Character.Inventory.AddNewToInventory(571, 1, InventoryType.Equipment, rndRare, 0, 0);
                                Session.Character.Inventory.AddNewToInventory(568, 1, InventoryType.Equipment, rndRare, 0, 0);
                    ...
                    if (MapInstance.IsReputationMap)
                    {
                        if (Group?.GroupType == GroupType.Group)
                        {
                            ...
                            {
                                targetSession.Character.GetReputation(monsterToAttack.Monster.Level);
                            }
                        }
                        else if (monsterToAttack.MonsterVNum == 157)
                        {
                            return;
                        }
                        else
                        {
                            ...
                            GetReputation(monsterToAttack.Monster.Level + (monsterToAttack.Monster.HeroLevel * 4));
                            GenerateFamilyXp(monsterToAttack.Monster.Level / 3);
                        }
                    }//keine ruf map
                    else
                    {
                        if (Group?.GroupType == GroupType.Group)
                        {
                            foreach (ClientSession targetSession in Group.Sessions.Where(s => s.Character.MapInstanceId == MapInstanceId))
                            {
                                targetSession.Character.GetReputation(monsterToAttack.Monster.Level + (monsterToAttack.Monster.HeroLevel *4));
                                GenerateFamilyXp(monsterToAttack.Monster.Level / 3);
                            }
                        }
                        else if (monsterToAttack.MonsterVNum == 157)
        ...
        {
          /*  int act1 = (ServerManager.RandomNumber()+ 10) * 20;
            int act2 = (ServerManager.RandomNumber() + 10) * 40;
            int act3 = (ServerManager.RandomNumber() + 10) * 80;
            int act4 = (ServerManager.RandomNumber() + 10) * 100;
            */
           
            if(mapMonster.MonsterVNum == 164 || mapMonster.MonsterVNum == 166)
            {
                return 5000;
            }
            if(mapMonster.MonsterVNum == 242 || mapMonster.MonsterVNum == 34)
            {
               return 9000;
            }
            if (mapMonster.MonsterVNum == 364 || mapMonster.MonsterVNum == 468)
            {
                return 14000;
            }
            if (mapMonster.MonsterVNum == 1055 || mapMonster.MonsterVNum == 1057)
            {
                return 23000;
            }
            if (mapMonster.MonsterVNum == 374 || mapMonster.MonsterVNum == 370)
            {
                return 30000;
            }
            if (mapMonster.MonsterVNum == 497 || mapMonster.MonsterVNum == 28)
            {
                return 39000;
            }
            if (mapMonster.MonsterVNum == 1409 || mapMonster.MonsterVNum == 621)
            {
                return 65000;
            }
            if (mapMonster.MonsterVNum == 2301 || mapMonster.MonsterVNum == 502)
            {
                return 80000;
            }
            if (mapMonster.MonsterVNum == 2522 || mapMonster.MonsterVNum == 2523)
            {
                return 105000;
            }
            if (mapMonster.MonsterVNum == 2512 || mapMonster.MonsterVNum == 2513)
            {
                return 120000;
            }
            if (mapMonster.MonsterVNum == 2559)
            {
                return 140000;
            }
            else
        ...
        private int GetHXP(MapMonster mapMonster, Group group)
        {
            ...
            if (HeroLevel > 50 && npcMonster.NpcMonsterVNum != 2559)
            {
                return 0;
            }
            if(HeroLevel < 50 && npcMonster.NpcMonsterVNum == 2559)
            ...
            double sharedHXp = npcMonster.HeroXp * (-13 * partySize + 113) / 100;//npcMonster.HeroXp / partySize;
        ...
        private int GetXP(MapMonster mapMonster, Group group)
        {
            NpcMonster npcMonster = mapMonster.Monster;
             ...
             if(npcMonster.HeroXp > 500  && Session.Character.Level < 95)
            {
                return 0;
            }
