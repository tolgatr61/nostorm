    ...
    public class SpecialItem : Item
    {
        ...
        public override void Use(ClientSession session, ref ItemInstance inv, byte Option = 0, string[] packetsplit = null)
        {
...

            {
                case 1785:
                    {
                        if (session.Character.HeroLevel >= 60 && !session.Character.IsMorphed && session.Character.Prestige < 6)
                        {
                            if (session.Character.IsUnequiped())
                            {
                                if (session.Character.HeroLevel >= 75 && session.Character.Prestige == 0)
                                {
                                    session.Character.Inventory.AddNewToInventory(9120, 1, InventoryType.Main, 0, 0, 0);
                                    session.Character.Prestige++;
                                    session.Character.HeroLevel = 0;
                                    session.Character.Level = 15;
                                    session.Character.JobLevel = 1;
                                    session.Character.Inventory.RemoveItemAmount(1785);
                                }
                                if (session.Character.HeroLevel >= 75 && session.Character.Prestige >= 0)
                                {
                                    session.Character.Prestige++;
                                ...
                                if(session.Character.HeroLevel >= 60 && session.Character.Prestige == 0)
                                {
                                    session.Character.Prestige++;
                                    session.Character.HeroLevel = 0;
                                    session.Character.Level = 15;
                ...
                case 5065:
                    int rnd = ServerManager.RandomNumber(0, 900);
                    if (rnd < 900)
                    {
                        short[] vnums =
                        {
                    ...
                    break;
                case 5070:
                    int rnd1 = ServerManager.RandomNumber(0, 900);
                    if (rnd1 < 900)
                    {
                        short[] vnums =
                        {
                    ...
                    break;
                case 5263:
                    int rnd2 = ServerManager.RandomNumber(0, 900);
                    if (rnd2 < 900)
                    {
                        short[] vnums =
                        {
                    ...
                    break;
                case 5274:
                    int rnd3 = ServerManager.RandomNumber(0, 900);
                    if (rnd3 < 900)
                    {
                        short[] vnums =
                        {
                ...
                case 5380:
                    {
                        ...
                        session.Character.Inventory.RemoveItemAmount(5380, 1);
                    }
                    break;
                case 5572:
                    {
            ...
            switch (Effect)
            {
                ...
                case 9850:
                    session.Character.ArenaWinner = 1;
                    session.SendPacket(UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey("TEST"), 0));
                    session.CurrentMapInstance?.Broadcast(session.Character.GenerateCMode());
                    session.SendPacket(session.Character.GenerateStat());
                    session.SendPackets(session.Character.GenerateStatChar());
                    session.Character.Inventory.RemoveItemFromInventory(inv.Id);
                        ...
                        default:
                            switch (VNum)
                            {
                                ...
                                case 9061:
                                    ...
                                    {
                                        session.Character.Family.FamilyLevel++;
