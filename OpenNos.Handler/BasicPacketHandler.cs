...
namespace OpenNos.Handler
{
    public class BasicPacketHandler : IPacketHandler
    {
        ...
        public void Hero(HeroPacket heroPacket)
        {
            ...
            {
                if (Session.Character.IsReputationHero() >= 3 && Session.Character.Reputation > 5000000 || Session.Character.Authority > AuthorityType.TGM)
                {
        ...
        public void Preq(PreqPacket packet)
        {
                        ...
                        && Session.Character.PositionX <= s.SourceX + 1) is Portal portal)
            {
                switch (portal.Type)
                {
                        ...
                        {
                            if (portal.DestinationMapId == 214)
                            {
                                if (Session.Character.HeroLevel <= 49)
                                {
                                    Session.SendPacket(UserInterfaceHelper.GenerateInfo(Language.Instance.GetMessageFromKey("You need to be HeroLvl 50")));
                                    Session.SendPacket(Session.Character.GenerateSay(Language.Instance.GetMessageFromKey("You need to be HeroLvl 50"), 10));
                                    return;
                                }
                            }
                            if (portal.DestinationMapId == 2605)
                            {
                                if (Session.Character.HeroLevel <= 25)
                                {
                                    Session.SendPacket(UserInterfaceHelper.GenerateInfo(Language.Instance.GetMessageFromKey("You need to be HeroLvl 25")));
                                    Session.SendPacket(Session.Character.GenerateSay(Language.Instance.GetMessageFromKey("You need to be HeroLvl 25"), 10));
                                    return;
                                }
                            }
                            if (portal.DestinationMapId == 2602)
                            {
                                if (Session.Character.HeroLevel < 1)
                                {
                                    Session.SendPacket(UserInterfaceHelper.GenerateInfo(Language.Instance.GetMessageFromKey("You need to be HeroLvl 1")));
                                    Session.SendPacket(Session.Character.GenerateSay(Language.Instance.GetMessageFromKey("You need to be HeroLvl 1"), 10));
                                    return;
                                }
                            }
                            break;
