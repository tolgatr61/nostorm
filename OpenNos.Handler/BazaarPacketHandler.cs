                            ...
                            if (newInv.Count > 0)
                            {
                                ...
                                if (ServerManager.Instance.Configuration.UseLogService)
                                {
                                    LogServiceClient.Instance.LogPacket(new PacketLogEntry()
                                    {
                                        ...
                                        Packet = $"[BAZAAR_BUY]BazaarId: {cBuyPacket.BazaarId} OldIIId: {bzcree.Item.Id} NewIIId: {newBz.Id} VNum: {cBuyPacket.VNum} Amount: {cBuyPacket.Amount} Rare: {newBz.Rare} Upgrade: {newBz.Upgrade} Price: {cBuyPacket.Price}"
                                    });
                                }
                            }
                        }
