    ...
    public class CharacterScreenPacketHandler : IPacketHandler
    {
        ...
        public void CreateCharacterAction(ICharacterCreatePacket characterCreatePacket, ClassType classType)
        {
                ...
                && (characterCreatePacket.Name.Length >= 4 && characterCreatePacket.Name.Length <= 14))
            {
                ...
                CharacterDTO characterDTO = new CharacterDTO
                {
                    ...
                    MapId = 2628,
                    MapX = ServerManager.RandomNumber<short>(95, 98),
                    MapY = ServerManager.RandomNumber<short>(70, 74),
                    MaxMateCount = 10,
                    MaxPartnerCount = 3,
                    SpPoint = 10000,
                    SpAdditionPoint = 0,
                    MinilandMessage = "Bienvenue",
                ...
                switch (characterDTO.Class)
                {
                    ...
                    default:
                        {
                            characterDTO.Level = 80;
                            characterDTO.JobLevel = 20;
                            characterDTO.HeroLevel = 1;
                            characterDTO.Hp = 221;
                            characterDTO.Mp = 221;
                        }
                        break;
                ...
                if (classType != ClassType.MartialArtist)
                {
                    ...
                    using (Inventory inventory = new Inventory(new Character(characterDTO)))
                    {
                        ...
                        inventory.AddNewToInventory(9009, 100, InventoryType.Main);
                        if (Session.Account.ReferrerId != 12)
                        {
                            inventory.AddNewToInventory(4134, 1, InventoryType.Equipment);
                            inventory.AddNewToInventory(8088, 1, InventoryType.Equipment);
                            inventory.AddNewToInventory(9065, 1, InventoryType.Main);
                            inventory.AddNewToInventory(4053, 1, InventoryType.Equipment);
                            inventory.AddNewToInventory(4053, 1, InventoryType.Equipment);
                            inventory.AddNewToInventory(8269, 1, InventoryType.Equipment);
                            inventory.AddNewToInventory(8271, 1, InventoryType.Equipment);
                            inventory.AddNewToInventory(8089, 1, InventoryType.Equipment); //9065 4053 8269
                            Session.Account.ReferrerId = 12;
                        }                       
        ...
        public void SelectCharacter(SelectPacket selectPacket)
        {
            try
            {
                ...
                if (!Session.Character.StaticBonusList.Any(p => p.StaticBonusType == StaticBonusType.AutoLoot))
                {
                    StaticBonusDTO staticBonusDTO = new StaticBonusDTO()
                    {
                    ...
                    if (DAOFactory.StaticBonusDAO.InsertOrUpdate(ref staticBonusDTO) != Data.Enums.SaveResult.Error)
                    {
                        Session.Character.StaticBonusList.Add(staticBonusDTO);
                    }
                }
