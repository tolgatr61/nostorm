        ...
        public void FamilyRepos(FReposPacket fReposPacket)
        {
            ...
            if (ServerManager.Instance.Configuration.UseLogService)
            {
                LogServiceClient.Instance.LogPacket(new PacketLogEntry()
                {
                    ...
                    Packet = $"[FAMILY_REPOS]OldIIId: {sourceInventory?.Id.ToString() ?? ""} NewIIId: {destinationInventory?.Id.ToString() ?? ""} ItemVNum: {(destinationInventory?.ItemVNum ?? -1)} Amount: {fReposPacket.Amount} Rare: {(destinationInventory?.Rare ?? -1)} Upgrade: {(destinationInventory?.Upgrade ?? -1)}"
                });
            }
        ...
        public void FamilyWithdraw(FWithdrawPacket fWithdrawPacket)
        {
            ...
            if (ServerManager.Instance.Configuration.UseLogService)
            {
                LogServiceClient.Instance.LogPacket(new PacketLogEntry()
                {
                    ...
                    SenderId = Session.Character.CharacterId,
                    PacketType = LogType.FamilyStorage,
                    Packet = $"[FAMILY_WITHDRAW]OldIIId: {previousInventory?.Id.ToString() ?? ""} NewIIId: {item2.Id} ItemVNum: {item2.ItemVNum} Amount: {fWithdrawPacket.Amount} Rare: {item2.Rare} Upgrade: {item2.Upgrade}"
                });
            }
