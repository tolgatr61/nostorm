...
namespace OpenNos.Handler
{
    public class InventoryPacketHandler : IPacketHandler
    {
        ...
        public void PutItem(PutPacket putPacket)
        {
            ...
            {
                Session.SendPacket(UserInterfaceHelper.GenerateMsg(Language.Instance.GetMessageFromKey("NOSVILLE_DROP"), 10));
                return;
            }
                ...
                case 41:
                    ...
                    if (specialist != null)
                    {
                        if (specialist.Rare != -2)
                        {
                            if (specialist.Item.EquipmentSlot == EquipmentType.Sp)
                            {
                                while (specialist.PerfectSP(Session)) ;
                            }
                        }
                        else
                        {
        ...
        public void Wear(WearPacket wearPacket)
        {
                ...
                == null)
            {
                ...
                if(inv?.Rare == 9)
                {
                    Session.SendPacket(Session.Character.GenerateSay("Your item is broken you can't wear it, you need to repair it first !", 11));
                    return;
                }
                if (inv?.Item != null)
                {
