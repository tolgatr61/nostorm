    ...
    public class NpcPacketHandler : IPacketHandler
    {
        ...
        public void ShowShop(RequestNpcPacket requestNpcPacket)
        {
            ...
            else
            {
                ...
                //Saber Stinkt
                if(npc.MapNpcId == 17249 && npc.MapId == 1)
                {
                    Session.SendPacket($"qna #n_run^8000^2^1^{npc.MapNpcId} There's a chance that your Artifact will get to a higher level (Your Artifact must be on first Slot in your Inventory)");
                    return;
                }

                if (npc.MapNpcId == 17210 && npc.MapId == 1)
                {
                    Session.SendPacket($"qna #n_run^8000^1^1^{npc.MapNpcId} If you continue you'll have a low % chance that your weapon will get to +11, but there's also a % chance that it dropps down to +9, the try itself costs 300kk gold");
                    return;
                }

                if (npc.MapNpcId == 17252 && npc.MapId == 1)
                {
                    Session.SendPacket($"qna #n_run^8000^3^1^{npc.MapNpcId} You can repair your item by spending 50kk");
                    return;
                }
                ...
                //else if (npc.Shop.Name == "+11 Upgrade")
                //{
                //    //notFinished
                //    Session.SendPacket($"qna #guri^8000 Upgrade your weapon to +11");
                //}
